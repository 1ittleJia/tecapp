...
public class ExportExcelUtil {
	...
	public  Workbook writeNewExcel(File file,String sheetName,List<PackExtractExpertEntity> lis) throws Exception{
		Workbook wb = null;
		Row row = null;
		Cell cell = null;
		...
		for (int i = 0; i < lis.size(); i++) {
			...
			PackExtractExpertEntity vo  = lis.get(i);
			if(null==vo){
				break;
			}
			//Cell赋值开始
			cell = row.createCell(0);
			cell.setCellValue(vo.getPackNm());
			cell.setCellStyle(cs);

			cell = row.createCell(1);
			cell.setCellValue(vo.getExpertNm());
			cell.setCellStyle(cs);
			...
			cell.setCellValue(vo.getExtractClass());
			cell.setCellStyle(cs);

			cell = row.createCell(4);
			cell.setCellValue(vo.getDelFlagNm());
			cell.setCellStyle(cs);
